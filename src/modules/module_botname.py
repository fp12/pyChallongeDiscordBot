...
class Module_BotName(Module):
    ...
    def build(self, json_data):
        ...
        if valid:
            ...
            log_modules.error('invalid data: ' + data)
        return valid
                    ...
                    for e in s['events']:
                        if e['name'] == event.name:
                            ...
                            if len(nickname) <= max_nickname_len and me.nick != nickname:
                                ...
                                if 'duration' in e:
                                    log_modules.info('Modules: Current nickname should be reverted in %ss' % e['duration'])
                                    self._client.loop.create_task(self._revert_to_default_in(int(e['duration'])))
