...
class ChallongeUsersDB:
    ...
    def _save(self):
        with open('data/users.json', 'w') as out_file:
            listToSave = [{k: line[k] for k in ('id', 'challonge_username', 'challonge_apikey')} for line in self._db]
    ...
    def dump(self):
        return utils.print_array('Challonge users database',
                                organizerFormat.format('Discord ID', 'Challonge Username', 'Account in use', 'API key set'),
                                self._db, 
                                lambda x: organizerFormat.format(x['id'],
                                                                 x['challonge_username'],
                                                                 'False' if x['account'] == None else 'True',
    ...
    def add(self, serverOwnerId):
        ...
        for x in self._db:
            if x['id'] == serverOwnerId:
                return
        if found == False:
            newUser = {'id':serverOwnerId, 'challonge_username':'', 'account':None, 'challonge_apikey':''}
    ...
    def set_username(self, id, username):
        ...
        for x in self._db:
            if x['id'] == id:
                ...
                found = True
        if found == False:
            self._db.append({'id':id, 'challonge_username':username, 'account':None, 'challonge_apikey':''})
        self._save()
            ...
            if x['id'] == server.owner.id:
                if x['account'] == None:
                    if x['challonge_username'] == '':
                        ...
                        x['account'] = Account(x['challonge_username'], encoder.decrypt(x['challonge_apikey']))
                        return  x['account']
                else:
                    return x['account']
        raise UserNotFound()
